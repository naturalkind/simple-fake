...
#    
        preds_1 = preds_1[0]
        mask_1 = np.zeros((body.shape[0], body.shape[1]), np.uint8)  
           
#        pnts_mouth_1 = [(preds_1[i,0], preds_1[i,1]) for i in range(48,60)]
        pnts_mouth_1 = [(preds_1[i,0], preds_1[i,1]) for i in range(0,68)]
        points_1 = np.array(pnts_mouth_1, np.int32)
        hull_1 = cv2.convexHull(points_1)
        body_cp = body.copy()
        imgs(body_cp)
        ...
        height, width = body.shape[:-1]
        lines_space_mask = np.zeros((height, width), np.uint8) 

#        pnts_mouth_1 = [(preds_1[i,0], preds_1[i,1]) for i in range(48,60)]
        points2 = np.array(pnts_mouth_1, np.int32)
        convexhull2 = cv2.convexHull(points2)
        body_cp = body.copy()
        for triangle in indexes_triangles:

            # Coordinates of the first person's delaunay triangles
            pt1 = pnts_mouth[triangle[0]]
            pt2 = pnts_mouth[triangle[1]]
            pt3 = pnts_mouth[triangle[2]]
            ...
            # Coordinates of the first person's delaunay triangles
            pt1 = pnts_mouth_1[triangle[0]]
            pt2 = pnts_mouth_1[triangle[1]]
            pt3 = pnts_mouth_1[triangle[2]]
        ...
        imgs(cv2.cvtColor(body_new_face, cv2.COLOR_BGR2RGB))  
        
          
        body_face_mask = np.zeros((body.shape[0], body.shape[1]), np.uint8) #np.zeros_like(body)
        body_head_mask = cv2.fillConvexPoly(body_face_mask, convexhull2, 255)
        body_face_mask = cv2.bitwise_not(body_head_mask)

        imgs(body_face_mask)
        print (body.dtype, body_face_mask.dtype)
        print (body.shape, body_face_mask.shape)
        ...
        imgs(cv2.cvtColor(seamlessclone, cv2.COLOR_BGR2RGB))
