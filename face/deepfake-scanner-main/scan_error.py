...
def get_frames(video, batch_size=10, target_fps=1):
	...
	batch = []
	print ("GET FRAMES", target_fps, nfrm, total)
	global h, w
...
def scan(file):
	...
	for batch in frames:
		...
		#print ("MMM", len(batch), len(results[0]), results[0][-1].shape)
...
#	tidx = 0
#	lnf = 0
#	out = cv2.VideoWriter('output.mp4', cv2.VideoWriter_fourcc(*'mp4v'), fps, (w,h))#(h,w)
#	print (len(all_frames))
#	for ivd in all_frames:
#		image = cv2.putText(ivd, str(preds[lnf]), (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA, False)
#		#imgs(image)
#		#print (image.shape)
