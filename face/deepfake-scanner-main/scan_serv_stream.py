...
def scan(file):
	...
	for ivd in all_frames:
		image = cv2.putText(ivd, str(preds[lnf]), (int(face_cord[lnf][2]+30.0), int(face_cord[lnf][3]+30.0)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA, False)
		image = cv2.rectangle(image, (face_cord[lnf][0], face_cord[lnf][1]), (face_cord[lnf][2], face_cord[lnf][3]), (255, 255, 255), 3)
		if tidx == int(fps):
			tidx = 0
			lnf += 1
		tidx += 1
...
def init(models_dir, cfg_file, dev):
	global device, mtcnn, facenet, deepware, margin, face_size, all_frames
	all_frames = []
...
class ImageWebSocket(tornado.websocket.WebSocketHandler):
	...
	def open(self):
		...
		print("WebSocket opened from: " + self.request.remote_ip,  self.clients)
	def on_message(self, message):
		ms =  json.loads(message)
		#print ("MSG >>>", ms)
		if list(ms.keys())[0] == "Start":
			self.myfile = open(ms["Start"]["Name"], "wb")
			self.namefile = ms["Start"]["Name"]
		...
		if list(ms.keys())[0] == "Done":
			...
			print (preds)
			self.write_message(json.dumps({"O":"Done",}))
			#test = get_frames(self.namefile, batch_size, scan_fps)
			#print (test)
#			if int(preds*100) > 20:
...
class MainHandler(tornado.web.RequestHandler):
	def get(self):
		self.render("upload_1.html", title="Нейронная сеть/Тренировка")
